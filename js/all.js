(async function() {
    ...
    if (betaCode in window.splus.beta_tests) {
        // Beta Enabled Notice
    ...
    function createBetaSection(name) {
        return createElement("div", ["splus-beta-section"], {
            id: `splus-beta-section-${name}`
        }, [
            createElement("h3", [], {
                textContent: name
            })
    ...
    function createBetaToggleCheckbox(name, onchange, checked = false, nestingLevel = 1) {
        return createElement("div", ["splus-beta-toggle"], {
            style: {
                paddingLeft: `${(nestingLevel - 1) * 10}px`
            }
        }, [
            createElement("label", [], {
                textContent: name
    ...
    setTimeout(function() {
        ...
        let dd = window.splus.Setting.getValue("defaultDomain");
        ...
        if (dd !== window.location.hostname && !DENYLISTED_DOMAINS.includes(window.location.hostname) && !window.location.hostname.match(/.*[-\.]app\.schoology\.com/)) {
            window.splus.Setting.setValue("defaultDomain", window.location.hostname, function() {
                ...
                if (bgColor && !["app.schoology.com", "lms.lausd.net"].includes(window.location.hostname)) {
                    ...
                    chrome.storage.sync.get({
                        ...
                        chrome.storage.sync.set({
                            ...
                            window.splus.Logger.log(`Schoology Plus has updated the domain on which it runs.\nPrevious: ${dd}\nNew: ${window.location.hostname}`);
                            location.reload();
                        });
                    });
                } else {
    ...
    if (bottom) {
        bottom.appendChild(createElement("span", ["footer-divider"], {
            ...
            createElement("span", ["footer-text-enhanced-by"], {
                ...
                onclick: () => window.open("https://schoologypl.us/?utm_source=ext-enhanced-by-footer", "_blank"),
    ...
    function generateDebugInfo() {
        return JSON.stringify({
            ...
            getBrowser: window.splus.getBrowser(),
            url: location.href,
    ...
    window.splus.modals = [
        new window.splus.Modal(
            ...
            window.splus.verboseModalFooterText,
            openOptionsMenu
        ),
        new window.splus.Modal(
            "changelog-modal",
        ...
        new window.splus.Modal(
            ...
            createElement("div", ["splus-modal-contents"], {}, [
                ...
                createElement("p", ["setting-description"], {
                    ...
                    createElement("span", [], {
                        textContent: "Schoology Plus would like to collect anonymous usage statistics to better understand how people use this extension. Per our "
                    }),
                ...
                createElement("p", ["setting-description"], {
                    ...
                    createElement("span", [], {
                        textContent: " so we can better understand how people use Schoology Plus, and we promise to be transparent about what we collect by providing aggregated statistics periodically in our "
                    }),
                    createElement("a", [], {
                ...
                createElement("p", ["setting-description"], {
                    ...
                    textContent: "You can change your choice at any point in Schoology Plus settings"
                }),
                createElement("div", ["settings-buttons-wrapper"], undefined, [
                    createButton("save-analytics-settings save-settings", "Save and Close", () => {
                        window.splus.Setting.saveModified();
                        window.splus.modalClose(document.getElementById("analytics-modal"));
                    })
                ])
            ]),
            window.splus.modalFooterText
        ...
        new window.splus.Modal(
            ...
            createElement("div", ["splus-modal-contents"], {}, [
                createElement("h2", ["setting-entry"], {
                    textContent: "Enable βeta Testing"
                }),
                createElement("p", ["setting-description"], {
                    style: {
                        fontSize: "14px"
                    ...
                    createElement("span", [], {
                        textContent: "If you have been given a Schoology Plus βeta code, you can enter it below to enable that beta test. If you don't know what this is, you should probably close this window, or you can "
                    }),
                    createElement("a", ["splus-track-clicks"], {
                    ...
                    createElement("span", [], {
                        textContent: " if you want to learn more."
                    }),
                ]),
                createElement("p", ["setting-description"], {
                    style: {
                    ...
                    createElement("strong", [], {
                        textContent: "You must"
                    }),
                    createElement("span", [], {
                        textContent: " have anonymous usage statistics enabled in order to participate in beta tests"
                    })
                ]),
                new window.splus.Setting(
                    "beta",
                ...
                createElement("p", ["setting-description"], {
                    ...
                    textContent: "You can change this setting at any point to disable or change the beta test. Access this page by pressing Alt+B (Option+B on Mac)."
                }),
                createElement("div", ["settings-buttons-wrapper"], undefined, [
                    createButton("save-beta-settings", "Save", () => {
                        ...
                        let current_test = window.splus.Setting.getValue("beta");
        ...
        new window.splus.Modal(
            ...
            createElement("div", ["splus-modal-contents"], undefined, [
                ...
                createElement("div", ["setting-entry"], {}, [
                    ...
                    createElement("p", ["setting-description"], {
                        textContent: "Lead developer"
                    })
                ]),
                createElement("div", ["setting-entry"], {}, [
                    createElement("h3", ["setting-title"], {}, [
                        createElement("a", [], {
                            href: "https://github.com/r58Playz",
                            textContent: "r58Playz (@r58Playz)"
                        })
                    ]),
                    createElement("p", ["setting-description"], {
                        textContent: "Maintains port to uBlock Origin and bookmarklet"
                    })
                ]),
                createElement("h2", ["setting-entry"], {
                    textContent: "Code Contributions"
                }),
                createElement("div", ["setting-entry"], {}, [
                    createElement("ul", ["contributor-list"], {
                        ...
                        innerHTML: (function(contribs) {
                            ...
                            name: "@Roguim",
                            url: "https://github.com/Roguim"
                        }, {
                            name: "Peter Stenger (@reteps)",
                            ...
                            name: "@KTibow",
                            url: "https://github.com/KTibow"
                        }, {
                            name: "@FenyLabs",
                            url: "https://github.com/FenyLabs"
                        }, {
                            name: "@jetline0",
                            url: "https://github.com/jetline0"
                        }, {
                            name: "@dsnsgithub",
                            url: "https://github.com/dsnsgithub"
                        }, {
                            name: "@senoj26",
                            url: "https://github.com/senoj26"
                        }, {
                            name: "@TheThonos",
                ...
                createElement("div", ["setting-entry"], {}, [
                    ...
                    createElement("p", ["setting-description"], {
                        innerHTML: 'Please see our <a href="https://github.com/aopell/SchoologyPlus/blob/develop/CONTRIBUTING.md">contributing guidelines</a> for various ways you can help in the development of Schoology Plus. Thanks for your interest in contributing!'
                    })
                ]),
                createElement("div", ["setting-entry"], {}, [
                    createElement("h3", ["setting-title"], {
        ...
        new window.splus.Modal(
            ...
            createElement("div", ["splus-modal-contents"], {}, [
                createElement("h2", ["setting-entry"], {
                    textContent: "Choose a New Theme!"
                }),
                createElement("p", ["setting-description"], {
                    textContent: "Schoology Plus has a bunch of new themes! Choose one from below, make your own, or keep your current theme. It's your choice! Click on each theme for a preview and then click the button to confirm your choice. You can change your theme at any time in Schoology Plus Settings.",
                    style: {
                        fontSize: "14px",
                        paddingBottom: "10px"
                    }
                ...
                createElement("div", ["splus-button-tile-container"], {}, [{
                    ...
                    text: "Modern Rainbow Theme",
                    theme: "Rainbow Modern",
                    new: true
                }, {
                    text: "Schoology Plus Classic Theme",
                    theme: "Schoology Plus",
                    active: window.splus.Theme.active.name === "Schoology Plus"
                }, {
                    text: `Keep Current Theme: ${window.splus.Theme.active.name}`,
                    theme: window.splus.Theme.active.name,
                    active: window.splus.Theme.active.name !== "Schoology Plus",
                    hidden: window.splus.Theme.active.name === "Schoology Plus"
                }, {
                ...
                (() => {
                    let btn = createButton("theme-popup-select-button", `Select Keep Current Theme: ${window.splus.Theme.active.name}`, e => {
                        ...
                        window.splus.modalClose(document.getElementById("choose-theme-modal"));
                        window.splus.Setting.setValue("theme", tempTheme);
    ...
    if (!newVersion || newVersion != chrome.runtime.getManifest().version) {
        ...
        if (window.splus.Setting.getValue("defaultDomain") != window.location.hostname) {
            window.splus.Logger.log("[Updater] Domain isn't set as default, skipping migrations until domain is updated.");
            return;
        }
                ...
                ['<button>View Changelog</button>', function(instance, toast) {
                    instance.hide({
                        ...
                        onClosing: function(instance, toast, closedBy) {
                            ...
                            window.splus.openModal("changelog-modal");
                        }
        ...
        await window.splus.versionSpecificFirstLaunch(currentVersion, newVersion);
        window.splus.Setting.setValue("newVersion", chrome.runtime.getManifest().version);
    }

    window.splus.Theme.profilePictureOverrides = [];
    let courseProfilePicOverrides = window.splus.Setting.getValue("forceDefaultCourseIcons") || {};

    let profilePicLoadTasks = [];

    for (let courseId in courseProfilePicOverrides) {
        if (courseProfilePicOverrides[courseId] == "enabled") {
            profilePicLoadTasks.push(window.splus.fetchApiJson("/sections/" + courseId));
        }
    }

    window.splus.Logger.log("Forcing Schoology-default icons for " + profilePicLoadTasks.length + " courses");
    ...
    for (let course of overrides) {
        window.splus.Theme.profilePictureOverrides.push({
    ...
    if (profilePicLoadTasks.length > 0) {
        window.splus.Theme.setProfilePictures();
    }
    ...
    document.body.onkeydown = (data) => {
        if (data.altKey && data.code === "KeyC") {
            ...
            window.splus.openModal("beta-modal");
        ...
        createElement(
            ...
            [
                createElement("div", ["explanation-popup"], {}, [
                    createElement("span", [], {
                        title: "",
                        textContent: "Use this button to temporarily disable your Schoology Plus theme if something isn't displaying correctly."
                    }),
                    createElement("h3", [], {
                        textContent: "OK",
                        onclick: e => {
                            ...
                            window.splus.Setting.setNestedValue("popup", "modernThemeToggle", false);
    ...
    for (let e of document.querySelectorAll(".close")) {
        e.onclick = window.splus.modalClose;
    }

    window.onclick = function(event) {
        if (window.splus.modals.find(x => x.element == event.target)) {
            window.splus.modalClose(event.target);
        }
    }
    ...
    function openOptionsMenu(settingsModal) {
        ...
        window.splus.updateSettings(() => {
            ...
            settingsModal.element.querySelector("#open-contributors").addEventListener("click", () => window.splus.openModal("contributors-modal"), {
                once: true
            });
    ...
    let coursesDropdownObserver = new MutationObserver(function(mutationList) {
        ...
        window.splus.Logger.log("Processing courses dropdown mutation");

        if (window.splus.Setting.getValue("archivedCoursesButton") === "show") {
        ...
        for (let contentTile of contentTiles) {
            ...
            if (cardData.parentElement.href) {
                ...
                if (courseLinkMatch && window.splus.Setting.getValue("courseAliases")) {
                    courseAlias = window.splus.Setting.getValue("courseAliases")[courseLinkMatch];
                }
            }
            ...
            let newCardDataChild = createElement("div", ["_36sHx", "_3M0N7", "fjQuT", "_1EyV_", "splus-coursesdropdown-nicknamed-dataset", "splus-addedtodynamicdropdown"], {}, [
                createElement("div", ["_3U8Br", "_1wP6w", "_23_WZ", "_2qcpH", "_3ghFm", "_17Z60", "_1Aph-", "gs0RB"], {
                    textContent: courseAlias
                }), // stylized like section title
                createElement("div", ["_2wOCj", "xjR5v", "_2qcpH", "_17Z60", "_1Aph-", "gs0RB", "splus-coursealiasing-exempt"], {
                    textContent: origCourseTitle.textContent + ": " + origSectionTitle.textContent
                }), // original full title, stylized like school name
                createElement("div", ["_2wOCj", "xjR5v", "_2qcpH", "_17Z60", "_1Aph-", "gs0RB"], {
        ...
        if (coursesHeader && !coursesHeader.querySelector(".splus-coursesdropdown-reorder-btn")) {
            ...
            let newBtn = createElement("img", ["splus-coursesdropdown-reorder-btn", "splus-addedtodynamicdropdown"], {
    ...
    coursesDropdownObserver.observe(coursesDropdownContainer, {
        childList: true,
        subtree: true
    });
    ...
    let groupsDropdownObserver = new MutationObserver(function(mutationList) {
        ...
        window.splus.Logger.log("Processing groups dropdown mutation");
    ...
    groupsDropdownObserver.observe(groupsDropdownContainer, {
        childList: true,
        subtree: true
    });
    ...
    let docObserver = new MutationObserver(function(mutationList) {
        ...
        window.splus.Theme.setProfilePictures(reprocessPictures);
    });
    ...
    window.splus.createQuickAccess = async function() {
        ...
        let wrapper = createElement("div", ["quick-access-wrapper"], {}, [
            createElement("h3", ["h3-med"], {
                title: "Added by Schoology Plus"
            }, [
                window.splus.createSvgLogo("splus-logo-inline"),
        ...
        const PAGES = [{
            textContent: "Grade Report",
            href: "/grades/grades",
            id: "quick-access-grades"
        }, {
            textContent: "Courses",
            href: "/courses",
            id: "quick-access-courses"
        }, {
            textContent: "Mastery",
            href: "/mastery",
            id: "quick-access-mastery"
        }, {
            textContent: "Groups",
            href: "/groups",
            id: "quick-access-groups"
        }, {
        ...
        try {
            let sectionsList = (await window.splus.fetchApiJson(`users/${window.splus.getUserId()}/sections`)).section;

            if (!sectionsList || sectionsList.length == 0) {
                wrapper.appendChild(createElement("p", ["quick-access-no-courses"], {
                    textContent: "No courses found"
                }));
            } else {
                ...
                for (let section of sectionsList) {
                    wrapper.appendChild(createElement("div", ["quick-access-course"], {}, [
                        ...
                        (courseIconsContainer = createElement("div", ["icons-container"], {}, [
                            ...
                            (courseOptionsButton = createElement("a", ["icon", "icon-settings", "splus-track-clicks"], {
                                href: "#",
                                dataset: {
                    ...
                    let quickLink = window.splus.Setting.getNestedValue("courseQuickLinks", section.id);
                    if (quickLink && quickLink !== "") {
                        courseIconsContainer.prepend(createElement("a", ["icon", "icon-quicklink", "splus-track-clicks"], {
                            href: quickLink,
                            title: `Quick Link \n(${quickLink})`,
                            dataset: {
                    ...
                    courseOptionsButton.addEventListener("click", () => window.splus.openModal("course-settings-modal", {
                        courseId: section.id,
    ...
    window.splus.indicateSubmittedAssignments = function() {
        ...
        async function isAssignmentCompleteAsync(assignmentId) {
            ...
            try {
                let revisionData = await window.splus.fetchApiJson(`dropbox/${assignmentId}/${window.splus.getUserId()}`);
                let revisions = revisionData.revision;

                return !!(revisions && revisions.length && !revisions[revisions.length - 1].draft);
            } catch (err) {
                window.splus.Logger.warn(`Couldn't determine if assignment ${assignmentId} was complete. This is likely not a normal assignment.`);
                return false;
            }
        }
        ...
        function isAssignmentMarkedComplete(assignmentId) {
            return !!window.splus.Setting.getNestedValue(completionOverridesSetting, assignmentId);
        }

        function setAssignmentCompleteOverride(assignmentId, isComplete) {
            isComplete = !!isComplete;

            let overrides = window.splus.Setting.getValue(completionOverridesSetting);
            ...
            window.splus.Setting.setValue(completionOverridesSetting, overrides);
        }
        ...
        async function processAssignmentUpcomingAsync(eventElement) {
            ...
            if (isMarkedComplete || await isAssignmentCompleteAsync(assignmentId)) {
                window.splus.Logger.log(`Marking assignment ${assignmentId} as complete ✔ (is force-marked complete? ${isMarkedComplete})`);
                eventElement.classList.add(assignCompleteClass);
            } else {
                eventElement.classList.add(assignIncompleteClass);
                window.splus.Logger.log(`Assignment ${assignmentId} is not submitted`);
            }
        ...
        async function indicateSubmitted() {
            window.splus.Logger.log("Checking to see if upcoming assignments are submitted");
            let idSet = new Set();
            for (let upcomingList of document.querySelectorAll(".upcoming-list")) {
                switch (window.splus.Setting.getValue("indicateSubmission")) {
                    case "disabled":
                        break;
                    case "strikethrough":
                ...
                for (let eventElement of upcomingEventElements) {
                    try {
                        ...
                        window.splus.Logger.error(`Failed checking assignment '${eventElement.querySelector(".infotip a[href]")?.href}' : `, err);
                    }
                }
            }
            ...
            if (reloadButton && reloadButton.offsetParent !== null) {
                ...
                if (overrides) {
                    ...
                    window.splus.Setting.setValue(completionOverridesSetting, overrides);
                    window.splus.Logger.info("Done clearing old overrides");
                }
            }
        }
