...
(async function() {
    ...
    (async function() {
            ...
            pdfjsLib.getDocument(filePath).then(function(pdf) {
                pdf.getPage(1).then(function(page) {
                    ...
                    page.render({
                        ...
                        window.splus.Logger.error("Unexpected PDF render error: " + e);
                    });
                }).catch(function() {
                    window.splus.Logger.log("could not open page 1 of document " + filePath + ". Not a pdf ?");
                });
            }).catch(function() {
                window.splus.Logger.log("could not find or open document " + filePath + ". Not a pdf ?");
            });
        }
        ...
        async function processDocumentElement(value) {
            ...
            $(value).find(".attachments-file-name>a").tipsy({
                gravity: "w",
                html: true,
                title: () => loadedTextHolder.dataset.tooltipHtml
            });
            ...
            registerGradeLoadHook(function(grades) {
                ...
                if (!grades.grades[assignmentId]) {
                    ...
                    if (assignment.exception == 1) {
                        ...
                        let gradeElem = wrapHtml(escapeForHtml((!assignment.max_points && assignment.grade >= 0 ? "+" : "") + assignment.grade), "span", {
                            class: "tooltip-grade-numerator"
                        });
                        if (assignment.max_points) {
                            gradeElem += " <span class=\"tooltip-horiz-divider\">/</span> ";
                            gradeElem += wrapHtml(escapeForHtml(assignment.max_points, "span", {
                                class: "tooltip-grade-denominator"
                            }));
                        } else {
                            gradeElem += " pts";
                        }
                        innerContent += wrapHtml(gradeElem, "span", {
                            class: "tooltip-grade-info"
                        });
                    } else {
                    ...
                    if (grades.gradeDrops[assignmentId]) {
                        // assignment dropped
                        innerContent = wrapHtml("Dropped", "span", {
                            class: "tooltip-dropped-indicator"
                        }) + " " + wrapHtml(innerContent, "span", {
                            class: "tooltip-dropped-gradeinfo"
                        });
                    }
                    ...
                    if (assignment.category_id && grades.categories[assignment.category_id]) {
                        footerElements.push(wrapHtml(escapeForHtml(grades.categories[assignment.category_id].title), "span", {
                    ...
                    if (grades.dropboxes[assignmentId]) {
                        ...
                        if (validSubmissions.length == 0) {
                            footerElements.push(wrapHtml("Not Submitted", "span", {
                                class: "tooltip-not-submitted"
                            }));
                        } else {
                            let submittedStatus = "Submitted";
                            if (validSubmissions[validSubmissions.length - 1].late) {
                                submittedStatus += " (Late)";
                                submittedStatus = wrapHtml(submittedStatus, "span", {
                                    class: "tooltip-submitted-late"
                                });
                            } else {
                                submittedStatus = wrapHtml(submittedStatus, "span", {
                                    class: "tooltip-submitted-ontime"
                                });
                            }
                    ...
                    if (footerElements.length > 0) {
                        innerContent += wrapHtml(footerElements.join("<span class=\"tooltip-horiz-divider\"> | </span>"), "p", {
                            class: "tooltip-footer"
                        });
                    }
                }

                loadedGrade = wrapHtml(innerContent, "div", {
                    class: "schoologyplus-tooltip assignment-tooltip"
                });
            });
        ...
        if (missingAssignmentCt > 0) {
            window.splus.Logger.log(`Fetched ${missingAssignmentCt} assignment(s) (${missingAssignmentErrorCt} error(s)) missing from summary API call`);
        }
        ...
        window.splus.Logger.log("Assignment data loaded, creating tooltips");
        ...
        window.splus.Logger.error("Error running materials page modification script: ", reason);
    });
