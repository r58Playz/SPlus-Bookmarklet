(async function() {
    ...
    await loadDependencies("theme-editor", ['theme-model']);
    ...
    const CURRENT_VERSION = window.splus.SchoologyTheme.CURRENT_VERSION;
     ...
     * @param {window.splus.SchoologyTheme} j A SchoologyPlus theme object
     */
    function importThemeFromObject(j) {
        ...
        return window.splus.SchoologyTheme.loadFromObject(j);
    }

    /**
     * Updates form elements with values from the provided theme
     * @param {window.splus.SchoologyTheme} t The theme to render
     */
    ...
    function updateOutput() {
        ...
        warnings = [];
        errors = [];
        theme = new window.splus.SchoologyTheme(themeName.value || undefined, window.splus.SchoologyTheme.CURRENT_VERSION);
        ...
        // Color
        theme.color = new window.splus.ThemeColor();
        if (themeColorHue.checked) {
            ...
            themeColorRainbowWrapper.classList.add("hidden");
            theme.color.custom = new window.splus.CustomColorDefinition(
            ...
            theme.color.rainbow = new window.splus.RainbowColorDefinition(new window.splus.RainbowColorComponentDefinition(), new window.splus.RainbowColorComponentDefinition(), new window.splus.RainbowColorComponentDefinition());
            ...
            if (colorRainbowHueAnimate.checked) {
                ...
                theme.color.rainbow.hue.animate = new window.splus.RainbowColorComponentAnimation(
        ...
        if (modernEnable.checked) {
            ...
            theme.color.modern = new window.splus.ModernColorDefinition();
            theme.color.modern.calendar = [];
            theme.color.modern.interface = new window.splus.ModernInterfaceColorDefinition();
            theme.color.modern.text = new window.splus.ModernTextColorDefinition();
            theme.color.modern.options = new window.splus.ModernOptionsDefinition();
    ...
    (() => {
        ...
        for (let t of window.splus.__defaultThemes) {
            ...
            if (!isLAUSD() && window.splus.LAUSD_THEMES.includes(t.name)) {
                continue;
            }
            allThemes[t.name] = t;
        }
        ...
        chrome.storage.sync.get(["theme", "themes"], s => {
            ...
            for (let t in allThemes) {
                let themeItem = createElement("a", ["collection-item", "theme-item"], {
                    ...
                    textContent: t + (window.splus.CLASSIC_THEMES.includes(t) ? " (Classic)" : ""),
                    dataset: {
                        tooltip: t + (window.splus.CLASSIC_THEMES.includes(t) ? " (Classic)" : "")
                    }
                })]);
    ...
    window.splus.Logger.debug("Finished loading theme-loader.js");
    window.splusLoaded.add("theme-loader");
})();
