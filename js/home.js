(async function() {
    ...
    function postFromBroadcast(broadcast) {
        let element = createElement("li", ["splus-broadcast-post"], {
            id: `broadcast${broadcast.id}`,
            timestamp: (broadcast.timestamp ? new Date(broadcast.timestamp).getTime() : Date.now()) / 1000
        }, [
                    ...
                    createElement("div", ["edge-left"], {}, [
                        createElement("div", ["picture"], {}, [
                            createElement("a", ["sExtlink-processed"], {
                                href: "",
                                title: "Schoology Plus Broadcast"
                            }, [
                                createElement("div", ["profile-picture-wrapper"], {}, [
                                    createElement("div", ["profile-picture"], {}, [
                                        createElement("img", ["imagecache", "imagecache-profile_sm"], {
                        ...
                        createElement("div", ["edge-footer"], {}, [
                            createElement("div", ["created"], {}, [
                                createElement("span", ["small", "gray"], {
                                    textContent: `${formatDateAsString(new Date(broadcast.timestamp || undefined))} | This post is pinned to the top`
                                })
                            ])
                        ])
                    ])
    ...
    if (homeFeedContainer && window.splus.Setting.getValue("broadcasts") !== "disabled") {
        (async function() {
            let observer = new MutationObserver(async function(mutations) {
                ...
                window.splus.Setting.setValue("unreadBroadcasts", unexpiredBroadcasts);
    ...
    function reorderSidebar() {
        ...
        let sidebarOrder = window.splus.Setting.getValue("sidebarSectionOrder");
        let excluded = sidebarOrder?.exclude || [];
        let included = (Array.from(sidebarOrder?.include || [])).reverse();

        for (let section of Array.from(window.splus.SIDEBAR_SECTIONS).reverse()) {
        ...
        for (let sectionName of included) {
            let section = window.splus.SIDEBAR_SECTIONS_MAP[sectionName];
            if (section) {
                let element = document.querySelector(section.selector);
                if (element) {
                    sidebar.insertAdjacentElement("afterbegin", element);
        ...
        try {
            ...
            let overdueHeading = document.querySelector(`${window.splus.SIDEBAR_SECTIONS_MAP["Overdue"].selector} h4`);
            overdueHeading?.replaceWith(createElement("h3", [], {
                style: {
                    textTransform: "capitalize"
                },
                textContent: overdueHeading.textContent.toLowerCase()
            }));
            let upcomingHeading = document.querySelector(`${window.splus.SIDEBAR_SECTIONS_MAP["Upcoming"].selector} h4`);
            upcomingHeading?.replaceWith(createElement("h3", [], {
                style: {
                    textTransform: "capitalize"
                },
    ...
    window.splus.Logger.debug("Finished loading home.js");
    window.splusLoaded.add("home");
})();
