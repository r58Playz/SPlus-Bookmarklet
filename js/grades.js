(async function() {
    ...
    function addEditDisableReason(err = "Unknown Error", causedBy403 = false, causedByNoApiKey = false) {
        ...
        window.splus.Logger.debug(editDisableReason, err, causedBy403, causedByNoApiKey);
    }
        ...
        items: {
            ...
            materials: {
                ...
                callback: function(key, opt) {
                    ...
                    window.open(`https://${window.splus.Setting.getValue("defaultDomain")}/course/${this[0].parentElement.id.match(/\d+/)[0]}/materials`, "_blank")
                }
            },
            updates: {
                name: "Updates",
                callback: function(key, opt) {
                    ...
                    window.open(`https://${window.splus.Setting.getValue("defaultDomain")}/course/${this[0].parentElement.id.match(/\d+/)[0]}/updates`, "_blank")
                }
            },
            student_grades: {
                name: "Grades",
                callback: function(key, opt) {
                    ...
                    window.open(`https://${window.splus.Setting.getValue("defaultDomain")}/course/${this[0].parentElement.id.match(/\d+/)[0]}/student_grades`, "_blank")
                }
            },
            mastery: {
                name: "Mastery",
                callback: function(key, opt) {
                    ...
                    window.open(`https://${window.splus.Setting.getValue("defaultDomain")}/course/${this[0].parentElement.id.match(/\d+/)[0]}/mastery`, "_blank")
                }
            },
            members: {
                name: "Members",
                callback: function(key, opt) {
                    ...
                    window.open(`https://${window.splus.Setting.getValue("defaultDomain")}/course/${this[0].parentElement.id.match(/\d+/)[0]}/members`, "_blank")
                }
            }
        }
    });
    ...
    var fetchQueue = [];
    (async function() {
        window.splus.Logger.log("Running Schoology Plus grades page improvement script");
        ...
        for (let course of courses) {
            courseLoadTasks.push((async function() {
                ...
                if (summary) {
                    ...
                    try {
                        let finalGradeArray = (await window.splus.fetchApiJson(`users/${window.splus.getUserId()}/grades?section_id=${courseId}`)).section[0].final_grade;
                        courseGrade = createElement("span", [], {
                            textContent: `${finalGradeArray[finalGradeArray.length - 1].grade.toString()}%`
                        });
                    } catch {
                        courseGrade = null;
                    }
                        ...
                        try {
                            ...
                            let processAssignment = async function(assignment) {
                                ...
                                if (assignment.querySelector(".missing")) {
                                    ...
                                    window.splus.Logger.log(`Fetching max points for assignment ${assignment.dataset.id.substr(2)}`);

                                    let json = await window.splus.fetchApiJson(`users/${window.splus.getUserId()}/grades?section_id=${courseId}`);
                                    ...
                                    if (!assignment.classList.contains("dropped")) {
                                        max += pts;
                                        window.splus.Logger.log(`Max points for assignment ${assignment.dataset.id.substr(2)} is ${pts}`);
                                    }
                                }
                            ...
                            for (let assignment of assignments) {
                                try {
                                    ...
                                    window.splus.Logger.error("Error loading assignment for " + courseId + ": ", assignment, err);
                                }
                            }
                            ...
                            if (assignments.length === 0) {
                                ...
                                let categoryArrow = createElement("img", ["expandable-icon-grading-report", "injected-empty-category-expand-icon"], {
                ...
                if (gradebookCourseContainerDiv != null && classTotal != 0 && periods.length === 1) {
                    ...
                    let gradeScale = window.splus.Setting.getValue("getGradingScale")(courseId);
                    let myPercentage = (classPoints / classTotal) * 100;

                    if (summaryPointsContainer) {
                        let gradeScaleSorted = Object.keys(gradeScale).sort((a, b) => b - a).map(function(p) {
                            return {
                                symbol: gradeScale[p],
                                minGrade: +p
                            };
                ...
                if (window.splus.isLAUSD()) { // To avoid making assumptions about how other schools use Schoology
        ...
        if (!document.location.search.includes("past") || document.location.search.split("past=")[1] != 1) {
            ...
            let timeRowLabel = createElement("label", ["modify-label"], {
                ...
                createElement("span", [], {
                    textContent: "Enable what-if grades"
                }),
            ...
            timeRow.appendChild(createElement("input", ["splus-track-clicks"], {
                ...
                onclick: function() {
                    ...
                    if (window.splus.Setting.getValue("apistatus") === "denied") {
                    ...
                    } else if (window.splus.Setting.getValue("apistatus") === "blocked") {
                        ...
                        let undroppedAssignItemSet = {
                            ...
                            calculateMinGrade: {
                                ...
                                callback: function(key, opt) {
                                    ...
                                    let gradingScale = window.splus.Setting.getValue("getGradingScale")(courseId);
        ...
        if (!document.location.search.includes("past") || document.location.search.split("past=")[1] != 1) {
            if (window.splus.Setting.getValue("orderClasses") == "period") {
        ...
        function queueNonenteredAssignment(assignment, courseId, period, category) {
            ...
            if (!noGrade && awardedGrade) {
                window.splus.Logger.log(`Found assignment (ID ${assignment.dataset.id.substr(2)}) with only letter-grade showing`);
            ...
            if (!noGrade) {
                window.splus.Logger.log(`Error loading potentially nonentered assignment with ID ${assignment.dataset.id.substr(2)}`);
                return;
            }
            ...
            if (noGrade && assignment.dataset.id) {
                ...
                let f = async () => {
                    ...
                    window.splus.Logger.log(`Fetching max points for (nonentered) assignment ${domAssignId}`);

                    let response = null;
                    let firstTryError = null;

                    try {
                        response = await window.splus.fetchApi(`sections/${courseId}/assignments/${domAssignId}`);
                    } catch (err) {
                        firstTryError = err;
                    }
                    ...
                    if (firstTryError) {
                        window.splus.Logger.log(`Error directly fetching max points for (nonentered) assignment ${domAssignId}, reverting to list-search`);
                    }
                    if (letterGradeOnly) {
                        window.splus.Logger.log(`Finding grade for letter-grade-only assignment ${domAssignId} from list-search`);
                    }

                    try {
                        response = await window.splus.fetchApi(`users/${window.splus.getUserId()}/grades?section_id=${courseId}`);
                        if (!response.ok) {
                            throw {
                                status: response.status,
                                error: response.statusText
            ...
            let injectedPercent = createElement("span", ["percentage-grade", "injected-assignment-percent"], {
                textContent: "N/A"
            });
        ...
        function setGradeText(gradeElement, sum, max, row, doNotDisplay) {
            if (gradeElement) {
                ...
                gradeElement.appendChild(createElement("span", ["rounded-grade"], {
                    textContent: doNotDisplay ? "" : Math.round(sum * 100) / 100
                }));
                gradeElement.appendChild(createElement("span", ["max-grade"], {
        ...
        function addLetterGrade(elem, courseId) {
            let gradingScale = window.splus.Setting.getValue("getGradingScale")(courseId);
            if (window.splus.Setting.getValue("customScales") != "disabled" && elem.textContent.match(/^\d+\.?\d*%/) !== null) {
            ...
            return function() {
                ...
                let submitFunc = function() {
                    ...
                    if (noGrade) {
                        ...
                        // ???
                        window.splus.Logger.warn("unexpected case of field type in editing grade");
                        return false;
                    }
        ...
        window.splus.Logger.log("Retrieving (" + fetchQueue.length + ") nonentered assignments info...")
        processNonenteredAssignments();
    }).catch(reason => {
        window.splus.Logger.error("Error running grades page modification script: ", reason);
    });
        ...
        if (fetchQueue.length > 0) {
            ...
            setTimeout(() => {
                func().then(x => {
                    ...
                    window.splus.Logger.warn("Caught error: ", err);
                    window.splus.Logger.log("Waiting 3 seconds to avoid rate limit");
                    if (err && err.firstTryError && err.firstTryError.status === 403) {
                        attempts = 100;
                    }
                    if (attempts > 3) {
                        window.splus.Logger.warn("Maximum attempts reached; aborting");
                    } else {
