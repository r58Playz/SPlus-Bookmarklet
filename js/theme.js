(async function() {
    ...
    await loadDependencies("theme", ["settings"]);
    ...
    class Theme {
        ...
        static getIcon(course) {
            ...
            if (window.splus.Setting.getValue("themes")) {
                let t = window.splus.Setting.getValue("themes").find(x => x.name === Theme.active.name);
            ...
            if (window.splus.Setting.getValue("useDefaultIconSet") === "enabled") {
        ...
        static hasBuiltInIcon(course) {
            for (let iconPattern of window.splus.icons) {
        ...
        static get active() {
            return tempTheme ? Theme.byName(tempTheme) : Theme.byName(window.splus.Setting.getValue("theme")) || Theme.byName("Schoology Plus");
        }
        ...
        static setModernColors(m) {
            window.splus.setCSSVariable("theme-is-dark", m.dark ? 1 : 0);
            document.documentElement.setAttribute("dark", m.dark);

            // Interface Colors
            window.splus.setCSSVariable("primary", m.interface.primary);
            window.splus.setCSSVariable("accent", m.interface.accent);
            window.splus.setCSSVariable("secondary", m.interface.secondary);
            window.splus.setCSSVariable("input", m.interface.input);
            window.splus.setCSSVariable("contrast-border", m.interface.border);
            window.splus.setCSSVariable("highlight", m.interface.highlight);
            window.splus.setCSSVariable("active", m.interface.active);
            window.splus.setCSSVariable("grades", m.interface.grades);
            window.splus.setCSSVariable("error", m.interface.error);

            // Calendar Colors
            for (let i = 0; i < 30; i++) {
                window.splus.setCSSVariable(`cal${i + 1}`, m.calendar[i]);
            }

            // Text Colors
            window.splus.setCSSVariable("text", m.text.primary);
            window.splus.setCSSVariable("muted-text", m.text.muted);
            window.splus.setCSSVariable("contrast-text", m.text.contrast);

            // Options
            window.splus.setCSSVariable("border-radius", `${m.options.borderRadius}px`);
            window.splus.setCSSVariable("border-size", `${m.options.borderSize}px`);
            window.splus.setCSSVariable("padding", `${m.options.padding}px`);
        }

        static setProfilePictures(candidateImages) {
            if (window.splus.Setting.getValue("courseIcons") === "disabled") return;
            ...
            if (candidateImages) {
                if (!skipOverriddenIcons) {
                    ...
                    pictures = Array.from(candidateImages).filter(x => x.src.match(window.splus.defaultCourseIconUrlRegex));
                }
            }
            ...
            if (skipOverriddenIcons && bigCourseIcon && !bigCourseIcon.src.match(window.splus.defaultCourseIconUrlRegex)) bigCourseIcon = null;
            ...
            if (coursesList && coursesList.length > 0) {
                ...
                for (let c of Array.from(coursesList).reduce((accum, curr) => accum.concat(Array.from(curr.querySelectorAll(".course-item"))), [])) {
                    ...
                    if (!img) {
                        continue;
                    } else if (skipOverriddenIcons && !img.src.match(window.splus.defaultCourseIconUrlRegex)) {
                        continue;
                    }
            ...
            for (let cDropIcon of coursesDropDownIcons) {
                ...
                if (!skipOverriddenIcons || existingUrl.match(window.splus.defaultCourseIconUrlRegex)) {
                    arrows.push(cDropIcon);
                }
            }

            let missingIconsLastCheck = window.splus.Setting.getValue("missingIconsLastCheck");
            ...
            for (let img of pictures) {
                ...
                if (window.splus.Setting.getValue("courseIconFavicons") !== "disabled" && img == bigCourseIcon && !document.querySelector("head > link[rel='icon'][type='image/svg+xml']")) {
        ...
        static setLogoUrl(url = "https://ui.schoology.com/design-system/assets/schoology-logo-horizontal-white.884fbe559c66e06d28c5cfcbd4044f0e.svg") {
            window.splus.setCSSVariable("background-url", `url(${url})`);
        }
    ...
    function beforeThisSemester(oldDate) {
        ...
        function getSemester(date) {
            ...
            return {
                semester: m > 0 && m < 8 ? "Spring" : "Fall",
                year: m == 0 ? y - 1 : y
            };
        }
    }
    window.splus.Theme = Theme;

    window.splus.Logger.debug("Loaded theme.js")
    window.splusLoaded.add("theme");
})();
